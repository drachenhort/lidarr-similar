...
- "Ignore" button per candidate row - persists to a new `IgnoreList` (SQLite, same store file) and is excluded from all future `discover_candidates()` runs via a new `ignored_names` parameter (filtered right after merge, before spending any enrichment API calls on it) as well as removed from the current view immediately. Verified live: an ignored artist stayed excluded even after a subsequent full discovery run.
